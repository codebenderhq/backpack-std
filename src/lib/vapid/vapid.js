import { MozCommon } from "./common.js";
...
class VapidCore {
  constructor(aud, sub, exp, lang, mzcc) {
    ...
    let enus = {
      ...
      errs: {
        ...
        ERR_PUB_R_KEY:
          "Invalid Public Key record. Please use a valid RAW Formatted record.",
        ERR_PUB_D_KEY:
          "Invalid Public Key record. Please use a valid DER Formatted record.",
        ERR_NO_KEYS:
          "No keys defined. Please use generate_keys() or load a public key.",
  ...
  import_public_raw(raw) {
    ...
    raw = raw.slice(-64);
    let x = this.mzcc.toUrlBase64(
      String.fromCharCode.apply(null, raw.slice(0, 32)),
    );
    let y = this.mzcc.toUrlBase64(
      String.fromCharCode.apply(null, raw.slice(32, 64)),
    );
...
export class VapidToken02 extends VapidCore {
  sign(claims) {
    return this._sign(claims)
      .then((elements) => {
        return {
